...
3. [사용 방법](#3-사용-방법)
4. [생성되는 하네스 구조](#4-생성되는-하네스-구조)
5. [문서 생성 기준](#5-문서-생성-기준)
6. [생성 파이프라인 상세](#6-생성-파이프라인-상세)
7. [프로파일 시스템](#7-프로파일-시스템)
8. [EHR4 vs EHR5 차이점](#8-ehr4-vs-ehr5-차이점)
9. [커스터마이징 범위](#9-커스터마이징-범위)
10. [플러그인 디렉토리 구조](#10-플러그인-디렉토리-구조)
...
| **안전 장치** | DB 변경 차단 훅 | `.claude/hooks/`, `settings.json` |
...
│   ├── hooks/
│   │   └── db-read-only.sh       ← DB SELECT-only 강제
│   ├── agents/
...
| 13 | `.claude/skills/procedure-tracer/SKILL.md` | ~300줄 | 프로시저 추적. |
| 14 | `.claude/skills/db-query/SKILL.md` | ~200줄 | DB 조회 가이드. |
| 15 | `.agents/skills/` | — | 위 5개 스킬의 Codex 호환 복사본. |
| 16 | `.codex/config.toml` | ~5줄 | Codex CLI 설정. |
| 17 | `.gitignore` | ~15줄 | 런타임 산출물 제외. |
