...
1. 변경된 파일 세트를 받아 B1~B7 경계면 매트릭스를 전부 점검한다
...
조건별 검증:
  auth_injection_methods == ["query_placeholder"]:
    매퍼에 $query 또는 ${query} 포함 확인
    Grep: "\$query" --glob="*-mapping-query.xml"
  auth_injection_methods == ["auth_table_join"]:
    매퍼에 INNER JOIN THRM1xx_AUTH 확인
    Grep: "JOIN\s+THRM[0-9]+_AUTH" --glob="*-mapping-query.xml"
  auth_injection_methods == ["auth_function_call"]:
    매퍼에 F_COM_GET_SQL_AUTH 호출 확인
    Grep: "F_COM_GET_SQL_AUTH" --glob="*-mapping-query.xml"
  여러 방식 포함: 하나 이상 확인
...
### B6: 법칙 D — ExecPrc.do ↔ CALL + ProDao (조건부, MISSING = HOLD)
```
...
### B7: 매퍼 XML 특수문자 — CDATA 준수
```
AGENTS.md.skel "매퍼 XML 특수문자" 규칙: 부등호(`<`, `<=`, `>=`, `<>`)·`&` 는
`<statement><![CDATA[...]]></statement>` 안에만, 엔티티 이스케이프 금지.

AI 자동 이스케이프/CDATA 누락 두 방향 모두 차단한다.

위반 시:
  - CDATA 없이 raw `<` → Anyframe 매퍼 파싱 실패로 해당 파일 전체 로딩 중단 → HOLD 판정
  - CDATA 안인데 `&lt;`/`&gt;`/`&amp;` 재이스케이프 → Oracle 에 엔티티 리터럴 전달 → FIX 판정

확인:
  (1) 변경된 매퍼 파일에 대해 XML well-formedness 검증:
      node -e "new (require('@xmldom/xmldom').DOMParser)({errorHandler:{error:e=>{throw e}}}).parseFromString(
                 require('fs').readFileSync(process.argv[1],'utf8'),'text/xml')" <file>
      (또는 xmllint --noout <file>)
  (2) 엔티티 이스케이프 감지:
      Grep: "&lt;|&gt;|&amp;(?!lt;|gt;|amp;|quot;|apos;)" --glob="*-mapping-query.xml"
      → 감지 시 해당 라인이 CDATA 밖(주석/속성)인지 확인. CDATA 안이면 FIX (엔티티 → raw + CDATA 유지).
  (3) CDATA 밖 raw 비교연산자 감지:
      수정된 매퍼에서 <![CDATA[ 범위 밖에 raw `<` 가 SQL 토큰으로 등장하면 HOLD.
```
...
점검:
  ...
  - ssnSearchType 분기 처리 누락 (session_vars 에 ssnSearchType 있을 때)
...
점검:
  - common_controllers 에 등록된 컨트롤러(GetDataListController/SaveDataController/ExecPrcController) 재구현 여부
  - auth_service_class 로직을 Controller에 인라인 구현 여부 (auth_service_class 있을 때만)
...
- B7 엔티티 이스케이프 재변환 (Oracle 로 `&lt;` 리터럴 전달)
...
- B7 CDATA 밖 raw `<` (Anyframe 매퍼 파일 전체 로딩 실패)
- 회귀 패턴 R-2(권한 필터 누락)
...
| B7 XML 특수문자 CDATA | PASS / FIX / HOLD | {파일:라인 — 이스케이프/raw 위반 근거} |
...
- **domain-knowledge 참조**: 법칙 C 판정 상세는 `.claude/skills/ehr-domain-knowledge/SKILL.md` §2 참조
