...
- 진입 조건: 매니페스트 존재 + schema_version ∈ {3, 4, 5} + 사용자 명시 audit 트리거
...
# Oracle + Tibero JDBC 드라이버 모두 검색
find . -name "ojdbc*.jar" -o -name "tibero*-jdbc.jar" -o -name "tibero*.jar" 2>/dev/null
```

→ 발견되면 URL에서 벤더 감지 후 `DB_CONNECTION` + `DB_VENDOR` 저장.

**벤더 감지 규칙:**
context.xml의 `url` 속성에서 `log4jdbc:` 를 제거한 순수 JDBC URL 기준:
- `jdbc:tibero:thin:@` 포함 → `DB_VENDOR="tibero"`
- `jdbc:oracle:thin:@` 포함 → `DB_VENDOR="oracle"`
- 그 외 → `DB_VENDOR="oracle"` (기본값)
...
echo "${DB_VENDOR:-oracle}" > .claude/skills/ehr-db-query/DB_VENDOR
```
...
if MANIFEST_FILE="$MANIFEST" node -e "
  ...
  if (m.schema_version !== 5) { throw new Error('unexpected schema_version: ' + JSON.stringify(m.schema_version) + ' (expected 5)'); }
  ...
  if (!m.ehr_cycle.learnings_meta || typeof m.ehr_cycle.learnings_meta !== 'object') { throw new Error('ehr_cycle.learnings_meta missing'); }
" 2>"$HS_VALIDATE_ERR"; then
  echo "✓ HARNESS.json 갱신 + 유효성 검증: plugin_version=$PLUGIN_VERSION, profile=$PROFILE, schema_version=5"
else
